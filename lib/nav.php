    ...
    $classes = array_unique($classes);
		...
		if ( $args->has_children && $this->nav_bar['item_type'] == 'li' ) {
			if ( $depth == 0 && $this->nav_bar['in_top_bar'] == false ) {
				$classes[] = 'has-flyout';
                                $toggle_sub_menu = 'data-toggle="dropdown" href="#" aria-haspopup="true" aria-expanded="false"';
		...
		$class_names = $class_names ? ' class="' . esc_attr( $class_names ) . ' ' . $toggle_li_class. '"' : '';
		if ( $depth > 0 ) {
		...
		$attributes .= ! empty( $item->xfn )        ? ' rel="'    . esc_attr( $item->xfn        ) .'"' : '';
                ...
                $attributes .= $toggle_sub_menu_class;
                $attributes .= $toggle_sub_menu;
